...
// 生成密钥函数，输入一个长度为11的密钥数组k，输出两个长度为9的密钥数组k1和k2
...
void f(int R[], int K[]) {
    ...
    int x1, x2, x3, x4;
    x1 = S1[s1[1] * 2 + s1[4]][s1[2] * 2 + s1[3]][0];
    x2 = S1[s1[1] * 2 + s1[4]][s1[2] * 2 + s1[3]][1];
    x3 = S2[s2[1] * 2 + s2[4]][s2[2] * 2 + s2[3]][0];
    x4 = S2[s2[1] * 2 + s2[4]][s2[2] * 2 + s2[3]][1];
    R[1] = x2;
    R[2] = x4;
    R[3] = x3;
    R[4] = x1;
